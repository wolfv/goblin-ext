...
use goblin::error;
use goblin::mach::constants::cputype::{CPU_TYPE_ARM64, CPU_TYPE_ARM64_32};
use goblin::mach::header::{filetype_to_str, Header};
use goblin::mach::load_command::*;
use alloc::collections::BTreeMap;
use alloc::string::String;
use alloc::vec::Vec;
use core::mem;
use scroll::{ctx::SizeWith, ctx::TryIntoCtx, Pread, Pwrite};
...
impl MachOWriter {
        ...
        for (idx, cmd) in self.load_commands.iter().enumerate() {
            if let CommandVariant::IdDylib(dylib_cmd) = &cmd.command {
                ...
                if name_offset < self.data.len()
                    && let Ok(s) = self.data.pread::<&str>(name_offset)
                {
                    return Some(s.into());
                }
            }
    ...
    pub fn change_dylib(&mut self, old_name: &str, new_name: &str) -> error::Result<()> {
        ...
        if !found {
            return Err(error::Error::Malformed(
                alloc::format!("Dylib '{}' not found", old_name),
            ));
        }
    ...
    pub fn delete_dylib(&mut self, path: &str) -> error::Result<()> {
        ...
        if let Some(idx) = found_idx {
            ...
            Err(error::Error::Malformed(
                alloc::format!("Dylib '{}' not found", path),
            ))
        }
    }
    ...
    pub fn add_dylib(&mut self, path: &str, kind: DylibKind) -> error::Result<()> {
        ...
        for (idx, cmd) in self.load_commands.iter().enumerate() {
            ...
            if existing_path == Some(path) {
                return Err(error::Error::Malformed(
                    alloc::format!("Dylib '{}' already exists", path),
                ));
            }
        }
        ...
        for (idx, cmd) in self.load_commands.iter().enumerate() {
            if let CommandVariant::Rpath(rpath_cmd) = &cmd.command {
                ...
                if existing_path == path {
                    return Err(error::Error::Malformed(
                        alloc::format!("Rpath '{}' already exists", path),
                    ));
                }
            }
        }
    ...
    pub fn delete_rpath(&mut self, path: &str) -> error::Result<()> {
        ...
        if let Some(idx) = found_idx {
            ...
            Err(error::Error::Malformed(
                alloc::format!("Rpath '{}' not found", path),
            ))
        }
    }
    ...
    pub fn change_rpath(&mut self, old_path: &str, new_path: &str) -> error::Result<()> {
        ...
        Err(error::Error::Malformed(
            alloc::format!("Rpath '{}' not found", old_path),
        ))
    }
    ...
    fn relocate_segments(&mut self, new_offset: usize) -> error::Result<()> {
        ...
        let page_size = self.page_size();
        let aligned_offset = new_offset.div_ceil(page_size) * page_size;
    ...
    fn serialize_load_command_at_index(&self, idx: usize) -> error::Result<Vec<u8>> {
        ...
        match cmd {
            ...
            _ => {
                ...
                pwrite_command_variant(cmd, &mut buf, self.ctx.le)?;
            }
        }
...
/// Write a command variant to a buffer using pwrite
/// Returns Ok(()) if successful, Err if this command type doesn't support pwrite
fn pwrite_command_variant(
    cmd: &CommandVariant,
    buf: &mut [u8],
    le: scroll::Endian,
) -> error::Result<()> {
...
mod codesign_impl {
    ...
    pub fn is_linker_signed(data: &[u8]) -> bool {
        ...
        use goblin::mach::load_command::LC_CODE_SIGNATURE;
    ...
    pub fn generate_adhoc_signature(
        ...
        sig.extend_from_slice(&blob_index.as_bytes());
