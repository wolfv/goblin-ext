#!/usr/bin/env python3
"""
Test framework for comparing goblin-ext's MachOWriter against Apple's install_name_tool.

This script tests various modifications on dylib files and compares the output
of goblin-ext's install_name_tool implementation against Apple's official tool.
...
def run_goblin_tool(
    ...
    """Run goblin-ext's install_name_tool."""
    cmd = [str(goblin_path)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Test goblin-ext MachOWriter against Apple's install_name_tool"
    )
    parser.add_argument(
        "path",
        type=Path,
